...
router.get('/', (req: Request, res: Response) => {
    res.send(products);
    console.log("passou por aqui. " + products);
});
...
router.post('/', (req: Request, res: Response) => {
   const body = req.body;
   if(Object.entries(body).length == 0 ) return res.status(401).send({message: 'O corpo veio vazio.'});
...
router.delete('/remove/:id', (req: Request , res: Response) => {
   ...
   if(productIndex === -1) return res.status(400).send({message: 'Produto não encontrado.'});
